...
const MENU: MenuItem[] = [
  {
    ...
    category: "버거",
    options: [{ label: "세트", choices: [{ key: "single", label: "단품" }, { key: "set", label: "세트(+2,000)", priceDelta: 2000 }] }],
  },
  {
    ...
    category: "음료",
    options: [{ label: "사이즈", choices: [{ key: "s", label: "S" }, { key: "m", label: "M(+500)", priceDelta: 500 }, { key: "l", label: "L(+1,000)", priceDelta: 1000 }] }],
  },
...
export default function OrderScreen() {
  ...
  return (
    <>
          ...
          <aside className="card" style={{ alignSelf: "start", position: "sticky", top: 76 }}>
            ...
            <div className="pad">
              ...
              <div className="flex" style={{ gap: 12, marginTop: 10 }}>
                ...
                <button className="btn primary" disabled={!cart.length} onClick={() => setIsDialog(true)}>주문하기</button>
              </div>
            </div>
          </aside>
        </div>
      ...
      {/* 주문 확인 모달 */}
      <div className={`dialog-backdrop ${isDialog ? "" : "hide"}`} onClick={() => setIsDialog(false)}>
        <div className="dialog" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex" style={{ gap: 12, justifyContent: "flex-end", marginTop: 12 }}>
            <button className="btn" onClick={() => setIsDialog(false)}>뒤로</button>
            <button className="btn primary" onClick={placeOrder}>결제하기</button>
          </div>
